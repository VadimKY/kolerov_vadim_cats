...
const btnCatDelete = document.querySelector('#delete');
...
function updateLocalStorage(data, action) { // {type: 'ADD_CATS} - функция обновления локал стоража
    const oldStorage = JSON.parse(localStorage.getItem('cats')); //берем котов
    ...
    switch (action.type) {
        ...
        case 'EDIT_CAT': // изменить данные 
            const updateStorage = oldStorage.map(cat => cat.id === data.id ? data : cat); //проеряем по id если совпадает то заменяем data
            localStorage.setItem('cats', JSON.stringify(updateStorage)); // на новый массив, если нет оставляем старый
            return;
        default:
            break;
    }
...
btnCatDelete.addEventListener('click', () => popupCatInfo.open());
btnCatDelete.addEventListener('submit', handleDeleteCat);
